...
const AdminUserpage = () => {
  const { data, isLoading } = useQuery({
    queryKey: ['USER'],
    queryFn: async () => {
      const { data } = await instance.get("/Admin/user/get-all")
      return data
    }
  })
              ...
              <tbody>
                {
                  data?.user.map((user: any, index: number) => (
                    <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                      ...
                      <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{user?.role ? "Admin" : "user"}</td>
                      <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{user?.createdAt}</td>
